...
# Keep LayerQuantConfig as a backward-compatible alias.
# New code should use LayerQuantSpec (a frozen dataclass) instead.
class LayerQuantConfig(dict):
    """Thin dict wrapper for backward compatibility.

    Consumers should migrate to :class:`LayerQuantSpec` which provides
    typed attribute access and is immutable.
    """
    ...
    def to_spec(self) -> LayerQuantSpec:
        """Convert this dict-based config to a typed :class:`LayerQuantSpec`."""
        return LayerQuantSpec(
            quant_type=self["quant_type"],
            quant_dtype=self["quant_dtype"],
            is_dynamic=self["is_dynamic"],
            quant_method=self.get("quant_method", None) or None,
        )


def _spec_to_layer_quant_config(spec: LayerQuantSpec) -> LayerQuantConfig:
    """Convert a :class:`LayerQuantSpec` back to a dict-style
    :class:`LayerQuantConfig` for backward-compatible callers."""
    return LayerQuantConfig(
        quant_type=spec.quant_type,
        quant_dtype=spec.quant_dtype,
        is_dynamic=spec.is_dynamic,
        quant_method=spec.quant_method or "",
    )
...
class QuantizationConfig:
    ...
    Primary API:
    - ``get_layer_quant_spec(prefix)`` -> :class:`LayerQuantSpec`
    - ``global_quant_spec`` property -> :class:`LayerQuantSpec`

    Backward-compatible API (returns dict-style :class:`LayerQuantConfig`):
    ...
    @property
    def global_quant_spec(self) -> LayerQuantSpec:
        """The default quantization spec for all layers."""
        return self._parsed.global_spec

    def get_layer_quant_spec(self, layer_name: str) -> LayerQuantSpec:
        """Return the :class:`LayerQuantSpec` for *layer_name*.

        Resolution order:
        1. Check exclude list -> ``LayerQuantSpec.no_quant()``.
        2. Exact match in ``parsed.layer_specs``.
        3. fnmatch-style pattern match in ``parsed.layer_pattern_specs``.
        4. Fall back to ``global_quant_spec``.
        """
        # 1. Exclude list
        if self._is_excluded(layer_name):
            return LayerQuantSpec(quant_dtype=self.torch_dtype)
    ...
    def global_quant_config(self) -> LayerQuantConfig:
        """Backward-compatible: returns a dict-style :class:`LayerQuantConfig`."""
        return _spec_to_layer_quant_config(self._parsed.global_spec)

    @global_quant_config.setter
    def global_quant_config(self, value):
        """Setter for backward compat -- accepts LayerQuantConfig or None."""
        if value is None:
            return
        if isinstance(value, LayerQuantConfig):
            self._parsed = ParsedQuantConfig(
                global_spec=value.to_spec(),
                layer_specs=self._parsed.layer_specs,
                layer_pattern_specs=self._parsed.layer_pattern_specs,
                exclude_layers=self._parsed.exclude_layers,
            )
        # Ignore other types silently for backward compat

    @property
    def layer_quant_config(self) -> dict[str, LayerQuantConfig]:
        """Backward-compatible: pattern specs as dict of LayerQuantConfig."""
        result = {}
        for pattern, spec in self._parsed.layer_pattern_specs:
            result[pattern] = _spec_to_layer_quant_config(spec)
        return result

    @layer_quant_config.setter
    def layer_quant_config(self, value: dict):
        """Setter for backward compat -- accepts dict of LayerQuantConfig."""
        if not value:
            self._parsed.layer_pattern_specs = []
            return
        specs = []
        for pattern, cfg in value.items():
            if isinstance(cfg, LayerQuantConfig):
                specs.append((pattern, cfg.to_spec()))
            elif isinstance(cfg, LayerQuantSpec):
                specs.append((pattern, cfg))
        self._parsed.layer_pattern_specs = specs

    def get_layer_quant_config(self, layer_name: str) -> LayerQuantConfig:
        """Backward-compatible: returns a dict-style :class:`LayerQuantConfig`.

        Prefer :meth:`get_layer_quant_spec` for new code.
        """
        spec = self.get_layer_quant_spec(layer_name)
        return _spec_to_layer_quant_config(spec)

    # -- convenience properties (delegate to global_quant_spec) -------------
