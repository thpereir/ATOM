...
- :class:`LayerQuantSpec` — a frozen dataclass replacing dict-based
  ``LayerQuantConfig`` for type-safe, immutable layer quant descriptions.
...
class LayerQuantSpec:
    ...
    @classmethod
    def no_quant(cls, dtype: Any = torch.bfloat16) -> LayerQuantSpec:
...
class ParsedQuantConfig:
    ...
    global_spec: LayerQuantSpec = field(default_factory=LayerQuantSpec)
    ...
    layer_pattern_specs: list[tuple[str, LayerQuantSpec]] = field(
...
def _build_quark_layer_spec(layer_dict: dict) -> LayerQuantSpec:
    """Build a :class:`LayerQuantSpec` from a single Quark per-layer dict."""
...
class QuarkParser(QuantConfigParser):
    ...
    def parse(self, hf_quant_config: dict) -> ParsedQuantConfig:
        ...
        global_spec = _build_quark_layer_spec(global_dict) if global_dict else LayerQuantSpec()

        pattern_specs: list[tuple[str, LayerQuantSpec]] = []
